...
QWidget* VentanaPrincipal::crearVistaHome() {
    ...
    // --- 2. FORMULARIO DE ALTA CON VALIDACIÓN ---
    ...
    le_inc_titulo->setPlaceholderText("Título de la incidencia (mínimo 3 caracteres)...");
    ...
    le_inc_zona->setPlaceholderText("Ubicación (ej. Aula 202)...");

    QPushButton *btnAlta = new QPushButton("ENVIAR INCIDENCIA");
    btnAlta->setObjectName("btnPrincipal");
    btnAlta->setFixedWidth(200);
    btnAlta->setEnabled(false); // Deshabilitado por defecto
    btnAlta->setCursor(Qt::ForbiddenCursor);
    btnAlta->setStyleSheet("QPushButton:disabled { background-color: #bdc3c7; color: #ecf0f1; }");
    ...
    tablaIncidencias->setAlternatingRowColors(true);
    tablaIncidencias->setSelectionBehavior(QAbstractItemView::SelectRows);
    tablaIncidencias->setEditTriggers(QAbstractItemView::NoEditTriggers); // No editable
    ...
    // --- LÓGICA DE VALIDACIÓN (REGEX / EMPTY) ---
    // Lambda para habilitar botón solo si los campos tienen contenido real
    auto validarFormulario = [this, btnAlta]() {
        ...
        bool zonaOk   = le_inc_zona->text().trimmed().length() >= 2;

        bool todoValido = tituloOk && descOk && zonaOk;

        btnAlta->setEnabled(todoValido);
        btnAlta->setCursor(todoValido ? Qt::PointingHandCursor : Qt::ForbiddenCursor);
    };

    connect(le_inc_titulo, &QLineEdit::textChanged, validarFormulario);
    connect(le_inc_desc,   &QLineEdit::textChanged, validarFormulario);
    connect(le_inc_zona,   &QLineEdit::textChanged, validarFormulario);
...
void VentanaPrincipal::crearIncidencia() {
    // 1. Captura de datos y limpieza de espacios (trimmed)
    QString titulo = le_inc_titulo->text().trimmed();
    QString desc = le_inc_desc->text().trimmed();
    QString zona = le_inc_zona->text().trimmed();
    QString tipo = cb_inc_tipo->currentText();

    // 2. Validación de campos vacíos (Regex o comprobación simple)
    // El Regex ^\s*$ detecta si el string está vacío o solo contiene espacios
    if (titulo.isEmpty() || desc.isEmpty() || zona.isEmpty()) {
        QMessageBox::warning(this, "Campos Incompletos",
            "Por favor, rellena todos los campos (Título, Descripción y Ubicación).");
        return;
    }

    // 3. Preparación de la URL y los parámetros
    ...
    // 4. El servidor espera el título en el body (según tu código previo)
    ...
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
            ...
            cargarIncidencias();
            QMessageBox::information(this, "Éxito", "Incidencia registrada correctamente.");
        } else {
            qDebug() << "Error de red:" << reply->errorString();
            QMessageBox::critical(this, "Error", "No se pudo conectar con el servidor.");
        }
        reply->deleteLater();
    });
}
