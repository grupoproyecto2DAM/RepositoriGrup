...
QWidget* VentanaPrincipal::crearVistaHome() {
    ...
    this->lblUser = new QLabel("👤 Usuario conectado: <b>Esperando login...</b>");
    ...
    le_inc_titulo->setPlaceholderText("Título de la incidencia...");
    le_inc_desc = new QLineEdit();
    le_inc_desc->setPlaceholderText("Descripción detallada...");
    ...
    // Inicialización correcta de punteros
    cb_inc_tipo = new QComboBox();
    cb_inc_tipo->addItems({"Electricidad", "Fontaneria", "Limpieza", "Climatizacion", "Mobiliario", "Tecnologias", "Seguridad", "Otro"});

    cb_inc_zona = new QComboBox();
    cb_inc_zona->addItems({"Edificio", "Planta", "Aula", "Laboratorio", "Baño", "Pasillo", "Patio", "Consejeria", "Otro"});

    fila2->addWidget(new QLabel("Tipo:"));
    fila2->addWidget(cb_inc_tipo);
    fila2->addSpacing(20);
    fila2->addWidget(new QLabel("Zona:"));
    fila2->addWidget(cb_inc_zona);

    QPushButton *btnAlta = new QPushButton("ENVIAR INCIDENCIA");
    btnAlta->setObjectName("btnPrincipal");
    btnAlta->setFixedWidth(200);
    btnAlta->setEnabled(false); // Deshabilitado hasta validar
    fila2->addStretch();
    ...
    // --- 4. FOOTER CON CERRAR SESIÓN ---
    QHBoxLayout *footerLayout = new QHBoxLayout();
    QLabel *lblStatus = new QLabel("✅ Sistema conectado al servidor");
    lblStatus->setStyleSheet("color: #27ae60; font-size: 11px;");
    ...
    // --- LÓGICA DE VALIDACIÓN Y CONEXIONES ---
    auto validarFormulario = [this, btnAlta]() {
        bool tituloOk = le_inc_titulo->text().trimmed().length() >= 3;
        bool descOk = le_inc_desc->text().trimmed().length() >= 5;
        btnAlta->setEnabled(tituloOk && descOk);
        btnAlta->setCursor(btnAlta->isEnabled() ? Qt::PointingHandCursor : Qt::ForbiddenCursor);
    };

    connect(le_inc_titulo, &QLineEdit::textChanged, validarFormulario);
    connect(le_inc_desc, &QLineEdit::textChanged, validarFormulario);
...
void VentanaPrincipal::crearIncidencia() {
    // 1. Captura de datos
    QString titulo = le_inc_titulo->text().trimmed();
    QString desc = le_inc_desc->text().trimmed();

    // Obtenemos los valores de los nuevos QComboBox
    QString zona = cb_inc_zona->currentText();
    QString tipo = cb_inc_tipo->currentText();

    // 2. Validación estricta
    if (titulo.isEmpty() || desc.isEmpty()) {
        QMessageBox::warning(this, "Campos Incompletos",
            "Por favor, asegúrate de rellenar el Título y la Descripción.");
        return;
    }

    // 3. Formato de fecha español (DD/MM/YYYY)
    QString fechaEsp = QDate::currentDate().toString("dd/MM/yyyy");

    // 4. Preparación de la URL y Query
    QUrl url("http://20.111.17.43/simagrow/incidencias");
    QUrlQuery query;
    query.addQueryItem("tipo", tipo);
    query.addQueryItem("zona", zona);
    query.addQueryItem("descripcion", desc);
    query.addQueryItem("fecha", fechaEsp);
    query.addQueryItem("alumnoNIA", QString::number(this->niaUsuarioLogueado));
    query.addQueryItem("estado", "ABIERTA");

    url.setQuery(query);

    // 5. Cuerpo del POST (título en JSON)
    QByteArray bodyData = "\"" + titulo.toUtf8() + "\"";
    ...
    connect(reply, &QNetworkReply::finished, [this, reply, fechaEsp]() {
        if (reply->error() == QNetworkReply::NoError) {
            // Limpieza de campos al tener éxito
            le_inc_titulo->clear();
            le_inc_desc->clear();

            // Reseteamos los QComboBox a la primera opción
            cb_inc_zona->setCurrentIndex(0);
            cb_inc_tipo->setCurrentIndex(0);

            // Recargar tabla para mostrar la nueva incidencia
            cargarIncidencias();

            QMessageBox::information(this, "SimaGrow", "¡Incidencia registrada correctamente el " + fechaEsp + "!");
        } else {
            qDebug() << "Error en POST:" << reply->errorString();
            QMessageBox::critical(this, "Error", "No se ha podido conectar con el servidor para guardar la incidencia.");
        }
        reply->deleteLater();
    });
}
